...
### 8.31 v7.26 PDF 报告整改 (2025-12-17) 🆕

#### 改进 8.31.1：PDF 报告 6 项整改

**用户需求**：
1. 封面：中英文靠近（不要空行）。生成时间前加"极致概念"。不要生成时间和冒号。不要版本
2. 目录：遗漏了"报告（极致概念）"
3. 章节分页：空两行连续输出，不要每个章节分页
4. 第四章与前端不一致
5. 第五章各角色输出没有正文
6. 元数据要与前端一致
...
```python
chapters = [
    {"title": "报告（极致概念）", "page": ""},  # 🆕 新增
    {"title": "用户原始需求", "page": ""},
    {"title": "校准问卷回顾", "page": ""},
    {"title": "需求洞察", "page": ""},
    {"title": "核心答案", "page": ""},
    {"title": "专家报告附录", "page": ""},      # 🆕 新增
    {"title": "执行元数据", "page": ""},
]
```
...
pdf.body_text("本报告由极致概念智能分析系统生成...")
```
