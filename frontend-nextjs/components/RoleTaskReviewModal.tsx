			...
			<div className="bg-white dark:bg-[var(--card-bg)] rounded-xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-gray-200 dark:border-[var(--border-color)]">
				...
				<div className="flex-1 overflow-y-auto px-6 py-6">
																		...
																		return (
																			<span
																				...
																				className={`text-xs px-2 py-1 rounded ${
																					isRecommended
																						? 'text-gray-700 dark:text-gray-300 bg-gray-200 dark:bg-gray-700 font-medium'
					...
					{isEditing && selectedAction === 'modify_tasks' && (
						<div className="info-box mt-4 flex items-start gap-3">
							<AlertCircle className="w-5 h-5 text-gray-600 dark:text-gray-400 flex-shrink-0 mt-0.5" />
							<div className="flex-1">
								<p className="text-sm text-gray-900 dark:text-gray-100 font-medium mb-1">编辑模式</p>
								<p className="text-sm text-gray-700 dark:text-gray-300">您可以修改、添加或删除任务。点击&ldquo;保存并继续&rdquo;提交修改。</p>
							</div>
						</div>
					)}
				</div>
					...
					{isEditing && selectedAction === 'modify_tasks' ? (
						...
						<div className="flex items-center justify-end gap-3">
							<button
								...
								className="px-4 py-2 text-gray-700 dark:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-full hover:bg-gray-50 dark:hover:bg-gray-800 transition flex items-center gap-2"
							>
