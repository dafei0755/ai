...
export const api = {
  ...
  async getUnifiedSessions(page: number = 1, pageSize: number = 20): Promise<{
    ...
    try {
      // 并行获取分析会话和搜索会话
      const [analysisData, searchData] = await Promise.all([
        this.getSessions(page, pageSize, true),
        this.getSearchSessions(pageSize * page, 0)  // 一次性获取足够数量
      ]);
      ...
      return {
        ...
        has_next: allSessions.length > startIdx + pageSize
      };
    } catch (error) {
