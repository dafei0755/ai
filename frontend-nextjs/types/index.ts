...
export interface StartAnalysisRequest {
  user_id: string;
  user_input: string;
  // 🆕 v7.158: 分析模式 - normal(深度思考) / search(深度搜索) / deep_thinking(深度思考pro)
...
export interface SessionListItem {
  ...
  user_input: string;
  pinned?: boolean;
  // v7.107: 分析模式和进度 - normal(深度思考) / deep_thinking(深度思考pro)
...
export interface StructuredReport {
  ...
  // 🔥 v7.39 概念图字段
  generated_images?: string[];  // 深度思考模式 - 集中式图片URL列表
...
export interface AnalysisReport {
  ...
  analysis_mode?: 'normal' | 'deep_thinking';  // 🆕 分析模式 - normal(深度思考) / deep_thinking(深度思考pro)
}
...
export interface EditableSearchStep {
  ...
  // 📊 v7.360: 动态增补标记
  is_supplementary?: boolean;    // 是否为动态增补的查询
  ...
  // 📊 v7.360: 板块关联（用于按板块分页）
  serves_blocks?: string[];      // 服务的板块ID列表
...
// ==================== Step 1 v3.0: Validation Report Types ====================
...
export interface BlockInfo {
  id: string;                    // 板块ID (block_1, block_2, ...)
  name: string;                  // 板块名称
}
