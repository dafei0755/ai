    ...
    async def initialize(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.info("✅ Playwright 浏览器池初始化成功")
...
class HTMLPDFGenerator:
    ...
    def _translate_all_english(self, text: str) -> str:
        ...
        sorted_translations = sorted(self.CONTENT_TRANSLATIONS.items(), 
    ...
    def translate_content(self, text: str) -> str:
        ...
        result = re.sub(r'(?:^|(?<=\s))([a-z][a-z0-9]*(?:_[a-z0-9]+)+)(?=\s*[:：]|\s*$)', 
                       replace_standalone_field, result, flags=re.MULTILINE)
    ...
    def _clean_and_format_value(self, value: Any, is_nested: bool = False) -> str:
        ...
        if isinstance(value, dict):
            # 跳过 confidence 字段
            filtered_dict = {k: v for k, v in value.items() 
                           if k.lower() not in ('confidence', '置信度', 'conf')}
    ...
    def _parse_deliverable_outputs(self, title: str, value: Any) -> Optional[Dict[str, Any]]:
            ...
            try:
                ...
                try:
                    ...
                    return {
                        "title": title,
                        "type": "text",
                        "content": self._simple_markdown(value)
                    }
        ...
        if isinstance(value, list):
            ...
            return {
                "title": title,
                "type": "text",
                "content": "<br><br>".join(formatted_parts)
            }
        ...
        if isinstance(value, dict):
            return {
                "title": title,
                "type": "text",
                "content": self._format_single_deliverable(value)
            }
    ...
    def parse_expert_content(self, expert_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if isinstance(content, str):
            ...
            sections.append({
                "title": "分析内容",
                "type": "text",
                "content": self._simple_markdown(translated)
            })
            ...
            sections.append({
                "title": "分析要点",
                "type": "list",
                "list_items": [self.translate_content(str(item)) for item in content]
            })
    ...
    def _parse_field(self, key: str, value: Any) -> Optional[Dict[str, Any]]:
        ...
        if isinstance(value, str):
                    ...
                    if isinstance(v, dict):
                        ...
                        for nk, nv in v.items():
                            ...
                            nested_fields.append({
                                "label": self.translate_label(nk),
                                "value": self._clean_and_format_value(nv)
                            })
                        if nested_fields:
                            fields.append({
                                "label": label,
                        ...
                        fields.append({
                            "label": label,
                            "value": self._clean_and_format_value(v)
                        })
                if fields:
                    return {
                        "title": title,
            ...
            if all(isinstance(item, str) for item in value):
                ...
                # 混合列表
                cleaned_items = [self._clean_json_artifacts(self.translate_content(str(item))) for item in value if item]
                return {
                    "title": title,
                    "type": "numbered_list",
                    "list_items": cleaned_items
        ...
        if loop is not None:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    asyncio.run,
                    self.generate_pdf_async(
                        experts, output_path, title, subtitle, session_id, **pdf_options
                    )
                )
