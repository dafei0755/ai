...
async def get_performance_summary(hours: int = Query(default=1, ge=1, le=24, description="统计时长（小时）")) -> Dict[str, Any]:
    """
    获取性能统计摘要
    ...
    filtered_records = [r for r in records if datetime.fromisoformat(r.get("timestamp", "")) > cutoff_time]
...
async def get_slow_requests(limit: int = Query(default=10, ge=1, le=100, description="返回记录数")) -> List[Dict[str, Any]]:
    """
    获取最近的慢请求（>1000ms）
...
async def get_performance_by_endpoint(
    ...
    for r in filtered_records:
        ...
        if key not in endpoint_stats:
            endpoint_stats[key] = {"path": key[0], "method": key[1], "count": 0, "durations": [], "slow_count": 0}
...
async def get_llm_summary(hours: int = Query(default=1, ge=1, le=24, description="统计时长（小时）")) -> Dict[str, Any]:
    """
    获取 LLM 调用统计摘要
    ...
    for r in filtered_records:
        ...
        if model not in by_model:
            by_model[model] = {"calls": 0, "success": 0, "tokens": 0, "duration": []}
...
async def get_recent_alerts(limit: int = Query(default=20, ge=1, le=100, description="返回记录数")) -> List[Dict[str, Any]]:
    """
    获取最近的告警记录
...
async def get_alert_stats(hours: int = Query(default=24, ge=1, le=168, description="统计时长（小时）")) -> Dict[str, Any]:
    """
    获取告警统计
    ...
    # 过滤时间范围
    cutoff_time = datetime.now() - timedelta(hours=hours)
    filtered_alerts = [a for a in alerts if datetime.fromisoformat(a.get("timestamp", "")) > cutoff_time]
...
async def get_llm_metrics() -> Dict[str, Any]:
    ...
    返回：
    - llm_active_calls: 当前正在执行的 LLM 调用数
    - llm_concurrency_limit: 全局并发上限（LLM_GLOBAL_CONCURRENCY 环境变量）
    """
