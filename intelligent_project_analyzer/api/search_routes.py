    ...
    async def event_generator():
        try:
            ...
            if request.phase_mode == "step1_only":
                # 仅执行 Step 1 分析，返回框架供用户编辑
                logger.info(f"📋 [v7.280] Step1 Only 模式")
                async for event in engine.execute_step1_only(
                    ...
                    if event_type in ("step1_complete", "awaiting_confirmation", "error"):
                        break
...
async def migrate_guest_sessions(
    ...
    try:
        ...
        guest_search_sessions = (
            ...
            .filter(
                (ArchivedSearchSession.user_id == None)
        ...
        for session in guest_search_sessions:
            ...
            date_part = (
                old_session_id.split("-")[1]
                if len(old_session_id.split("-")) > 1
                else datetime.now().strftime("%Y%m%d")
            )
            uuid_part = old_session_id.split("-")[-1] if len(old_session_id.split("-")) > 2 else uuid.uuid4().hex[:8]
            user_id_short = user_id[:8] if len(user_id) > 8 else user_id
            new_session_id = f"{user_id_short}-{date_part}-{uuid_part}"
