...
def sync_archdaily_incremental(days: int = 7):
    ...
    try:
        ...
        logger.success(f"✅ Archdaily 增量同步完成，状态: {status}") if success else logger.error(f"❌ Archdaily 增量同步失败")
...
def sync_gooood_incremental(days: int = 7):
    ...
    try:
        ...
        status = "success" if success else "failed"
        logger.success(f"✅ Gooood 增量同步完成，状态: {status}") if success else logger.error(f"❌ Gooood 增量同步失败")
