...
class ExpertConfig:
    ...
    tools: List[str]  # ["milvus"], ["tavily", "arxiv", "milvus"]  # v7.154: ragflow → milvus
...
class StrategyGenerator:
    ...
    #  v7.156: 凡有搜索，必有博查 - 所有搜索角色添加 bocha
    EXPERT_CONFIGS = {
        "V3": ExpertConfig(
            prefix="V3_",
            name="叙事与体验专家",
            agent_type="v3_narrative_expert",
            tools=["bocha", "tavily", "milvus"],  #  v7.156: 添加 bocha
            duration="30-40秒",
        ...
        "V4": ExpertConfig(
            ...
            tools=["bocha", "tavily", "arxiv", "milvus"],  #  v7.156: 添加 bocha
            duration="30-40秒",
        ...
        "V2": ExpertConfig(
            ...
            tools=["milvus"],  # V2 仅知识库（高层决策者，不做外部搜索）
            duration="30-40秒",
        ...
        "V7": ExpertConfig(
            prefix="V7_",
            name="情感洞察专家",
            agent_type="v7_emotional_insight",
            tools=["bocha", "tavily", "arxiv", "milvus"],  #  v7.156: 情感洞察专家
            duration="30-40秒",
            work_focus_description="基于第一批专家的分析结果，进行心理与情感分析，研究用户情感需求和心理动机，提供情感化设计建议",
        ),
    }
    ...
    def generate_strategy_preview(
        ...
        search_queries = self._generate_search_queries(expert_type, project_task, character_narrative)

        # 4. 生成工作重点（动态生成）
        work_focus = self._generate_work_focus(
            expert_type, config, available_deps, project_task, character_narrative, dependencies
        )
    ...
    def _extract_agent_insights(
        self, agent_results: Dict[str, Any], agent_key: str, agent_label: str
    ) -> Dict[str, Any]:
        """提取单个专家的关键洞察"""
        if agent_key not in agent_results:
            return {"available": False, "message": f"{agent_label}分析结果未找到"}
    ...
    def _generate_work_focus(
        ...
        if not self.llm_model:
            ...
            return [f"基于{deps_str}的分析结果，{config.work_focus_description}", "研究相关领域的最新趋势和最佳实践", "整合多方信息形成专业建议"]
        ...
        try:
            ...
            if available_deps:
                deps_summary = "\n".join(
                    [f"- {dep}: {dependencies.get(f'{dep}_', {}).get('summary', '已完成分析')}" for dep in available_deps]
                )
            else:
...
"""
            ...
            messages = [SystemMessage(content=f"你是{config.name}，擅长根据项目需求制定具体的工作计划。"), HumanMessage(content=prompt)]
    ...
    def _generate_search_queries(self, expert_type: str, project_task: str, character_narrative: str) -> Dict[str, str]:
        """生成搜索查询 - 使用LLM完全动态生成"""
        if not self.llm_model:
        ...
        try:
            # 专家角色描述（仅用于提示LLM）
            role_descriptions = {
                "V3": {"role": "叙事与体验专家", "purpose": "叙事设计、用户体验和情感触点", "search_focus": "空间叙事、用户旅程、情感体验"},
                "V4": {"role": "设计研究员", "purpose": "设计研究、方法论和行业案例", "search_focus": "设计研究、学术文献、创新方法论"},
                "V5": {"role": "场景与行业专家", "purpose": "行业趋势、场景分析和用户生态", "search_focus": "行业趋势、市场分析、用户行为"},
                "V2": {"role": "设计总监", "purpose": "设计方案、趋势和最佳实践", "search_focus": "设计案例、空间设计、行业标杆"},
                "V6": {"role": "专业总工程师", "purpose": "项目管理、实施方法和工程管理", "search_focus": "项目管理、施工技术、工程实施"},
            }
...
"""
    ...
    def _generate_search_queries_fallback(
        self, expert_type: str, project_task: str, character_narrative: str
    ) -> Dict[str, str]:
        """降级方案：基于关键词提取生成搜索查询"""
        # 从项目任务中提取关键词
        keywords = []
        ...
        queries = query_patterns.get(
            expert_type, [f"{base_keywords} 设计", f"{base_keywords} 研究 {current_year}", f"{base_keywords} design trends"]
        )

        return {"query_1": queries[0], "query_2": queries[1], "query_3": queries[2]}
