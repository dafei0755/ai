...
class CoreAnswerV7(BaseModel):
    ...
    deliverable_answers: List[DeliverableAnswer] = Field(
        ...
        description="各交付物的责任者答案列表（按优先级排序）"
    )
    expert_support_chain: List[ExpertSupportChain] = Field(
        default_factory=list,
        description="专家支撑链（非owner专家的贡献）"
    )
...
class FinalReport(BaseModel):
    ...
    insights: Optional[InsightsSection] = Field(
        default=None,
        description="[已废弃] 从所有专家分析中提炼的核心洞察（不再使用）"
    )
    ...
    def execute(
        ...
        try:
            ...
            self._update_progress(
                state,
                f"调用LLM整合{agent_count}位专家的分析结果（预计60-90秒）",
                0.3
            )
            ...
            if deliverable_metadata:
                ...
                if extracted_core_answer.get("deliverable_answers"):
                    ...
                    logger.info(f"✅ [v7.0] 已用责任者答案覆盖 core_answer: {len(extracted_core_answer['deliverable_answers'])} 个交付物")
                else:
            ...
            if isinstance(red_review, dict):
                ...
                for imp in improvements:
                    red_team_challenges.append({
                        ...
                        "status": "已修复" if imp.get("issue_id") in [s.get("issue_id") for s in improvement_suggestions] else "待处理",
...
"""
    ...
    def _analyze_questionnaire_insights(self, responses: List[Dict[str, Any]]) -> str:
        ...
        insights = [
            "## 用户需求关键洞察",
            "",
            f"基于{len(responses)}个问题的深入访谈，我们提取了以下关键洞察：",
            ""
        ]
