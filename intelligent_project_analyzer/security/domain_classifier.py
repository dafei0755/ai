...
class DomainClassifier:
    ...
    def classify(self, user_input: str) -> Dict[str, Any]:
        ...
        if self.llm_model:
            try:
                ...
                logger.info(
                    f" LLM判断: is_design={llm_result['is_design']}, confidence={llm_result.get('confidence', 0):.2f}"
                )
        ...
        if not llm_result.get("is_design", True) and llm_result.get("confidence", 0) > 0.8:
            return _non_design_response(confidence=llm_result.get("confidence", 0.9), reason="LLM判断该内容不属于空间设计领域")

        # 规则2: 非设计特征明显
        if non_design_strength >= 3 and design_strength == 0:
            return _non_design_response()
        if (non_design_strength >= 2 and non_design_strength - design_strength >= 2) or (
            non_design_strength >= 2 and design_strength <= 2 and len(design_stats["keywords"]) <= 1
        ):
            return _non_design_response()

        # 规则3: 设计特征明显
        if design_strength >= 3 and llm_result.get("is_design", True):
            confidence = max(self._compute_confidence(design_stats), llm_result.get("confidence", 0.0))
        ...
        return {
            ...
            "suggested_questions": ["您是否需要进行空间设计方面的分析？", "这个项目是否涉及建筑、室内或景观设计？", "您希望我帮您设计什么类型的空间？（如办公室、展厅、店铺等）"],
        }
    ...
    def _analyze_keywords(self, text: str, keyword_dict: Dict[str, List[str]]) -> Dict[str, Any]:
        ...
        return {"hits": total_hits, "categories": categories, "keywords": list(matched_keywords), "strength": strength}
...
- "写代码" → false（编程）"""
    ...
    def assess_task_complexity(self, user_input: str) -> Dict[str, Any]:
        ...
        complex_patterns = {
            ...
            # 3. 竞标/对标（解决#24/#16误判）
            "商业竞争": [r"竞标", r"投标", r"对标", r"对手", r"竞争", r"PK", r"差异化.*竞争", r"市场.*标杆"],
    ...
    def _recommend_experts_for_simple_task(self, user_input: str) -> List[str]:
        ...
        if any(kw in text for kw in ["命名", "起名", "取名", "诗词", "诗意", "文化", "传统", "禅意", "意境"]):
            experts.extend(["V3_叙事与体验专家_3-2", "V4_设计研究员_4-1"])  # 品牌叙事专家（擅长命名、品牌故事）  # 设计研究员（擅长案例对标）
        ...
        if not experts:
            experts = ["V3_叙事与体验专家_3-2", "V4_设计研究员_4-1"]
