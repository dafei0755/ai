...
⚠️ DEPRECATED (v9.1): 该组件未被当前 StateGraph Agent v7.17+ 调用。
Phase2 的 L2 透镜选择已内化到 requirements_analyst_agent 的 LLM prompt 中。
保留本文件供参考和未来可能的独立透镜服务复用。
...
class L2PerspectiveActivator:
        ...
        if os.path.exists(default_path):
            try:
                with open(default_path, "r", encoding="utf-8") as f:
                    ...
                    if rules:
                        logger.info(" [L2 Activator] Loaded activation rules from default path")
                        return rules
            except Exception as e:
    ...
    def format_activation_summary(self, active_perspectives: List[str]) -> str:
        ...
        summary = "**L2 Perspectives Activated**:\n"
        ...
        if extended:
            ...
            summary += "- Extended: None (base perspectives sufficient)\n"
