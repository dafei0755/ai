...
class RequirementsValidator:
    ...
    def _validate_l6_assumption_audit(
        self,
        data: Dict[str, Any],
        result: ValidationResult
    ):
        ...
        if len(assumptions) < 3:
            result.add_error(
                f"L6 assumption audit incomplete: found {len(assumptions)} assumptions, need at least 3"
            )
        ...
        if incomplete_assumptions:
            for idx, missing in incomplete_assumptions:
                result.add_warning(
                    f"L6 assumption {idx} missing fields: {', '.join(missing)}"
                )
        ...
        if shallow_assumptions:
            result.add_warning(
                f"L6 assumptions {shallow_assumptions} appear shallow or incomplete"
            )

        result.validation_details["l6_quality"] = "good" if not incomplete_assumptions and not shallow_assumptions else "needs_improvement"

    def _validate_l7_systemic_impact(
        self,
        data: Dict[str, Any],
        result: ValidationResult
    ):
        ...
        if missing_terms:
            result.add_error(
                f"L7 systemic impact incomplete: missing time dimensions {missing_terms}"
            )
        ...
        if not unintended or len(unintended) < 2:
            result.add_warning(
                f"L7 should identify at least 2 unintended consequences (found {len(unintended)})"
            )
        ...
        result.validation_details["l7_quality"] = "good" if not missing_terms and len(unintended) >= 2 else "needs_improvement"

    def _validate_human_dimensions(
        self,
        data: Dict[str, Any],
        result: ValidationResult
    ):
        ...
        for dim in dimensions:
            ...
            if not value or len(value) < 20:
                ...
                if generic_found:
                    result.add_warning(
                        f"{dim} contains generic phrases: {', '.join(generic_found[:3])}"
                    )
