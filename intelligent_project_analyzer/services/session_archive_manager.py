...
class ArchivedSearchSession(Base):
    ...
    deep_analysis_result = Column(Text, nullable=True)  # 深度分析结果 JSON
    four_missions_result = Column(Text, nullable=True)  # 4条使命结果 JSON
    quality_assessment = Column(Text, nullable=True)  # 答案质量评估 JSON
    answer_thinking_content = Column(Text, nullable=True)  # 答案思考过程
...
class SessionArchiveManager:
    ...
    @staticmethod
    def _remove_base64_from_dict(obj: Any, path: str = "") -> int:
        """
         v7.402: 递归清理字典中的 base64 图片数据
        
        Args:
            obj: 要清理的对象（dict/list）
            path: 当前路径（用于日志）
        ...
        if isinstance(obj, dict):
            for key, value in list(obj.items()):
                ...
                # 检查 image_url 和 url 字段
                if key in ["image_url", "url"] and isinstance(value, str):
                    if value.startswith("data:image/") and ";base64," in value:
                        ...
                        if match:
                            ...
                            obj[key] = f"[REMOVED_BASE64_{mime_type}_{original_size}_bytes]"
            ...
            for idx, item in enumerate(obj):
                cleaned = SessionArchiveManager._remove_base64_from_dict(item, f"{path}[{idx}]")
                total_cleaned += cleaned
        
        return total_cleaned

    def _clean_session_data_for_archive(self, session_data: Dict[str, Any]) -> Dict[str, Any]:
        """
         v7.402: 清理会话数据准备归档
        
        移除 events 和其他字段中的 base64 图片数据，避免数据库膨胀
        ...
        # 清理 events 数组
        if "events" in cleaned_data and isinstance(cleaned_data["events"], list):
            events_cleaned = self._remove_base64_from_dict(cleaned_data["events"], "events")
            if events_cleaned > 0:
                logger.info(f"   [v7.402] 从 events 清理 {events_cleaned / (1024 * 1024):.2f} MB base64")
                total_cleaned += events_cleaned
        
        # 清理 final_report
        if "final_report" in cleaned_data and isinstance(cleaned_data["final_report"], dict):
            report_cleaned = self._remove_base64_from_dict(cleaned_data["final_report"], "final_report")
            if report_cleaned > 0:
                logger.info(f"   [v7.402] 从 final_report 清理 {report_cleaned / (1024 * 1024):.2f} MB base64")
                total_cleaned += report_cleaned
        
        # 清理 agent_results
        if "agent_results" in cleaned_data and isinstance(cleaned_data["agent_results"], dict):
            results_cleaned = self._remove_base64_from_dict(cleaned_data["agent_results"], "agent_results")
            if results_cleaned > 0:
                logger.info(f"   [v7.402] 从 agent_results 清理 {results_cleaned / (1024 * 1024):.2f} MB base64")
                total_cleaned += results_cleaned
        
        if total_cleaned > 0:
            logger.success(f" [v7.402] 归档前清理完成，节省 {total_cleaned / (1024 * 1024):.2f} MB")
    ...
    async def archive_session(self, session_id: str, session_data: Dict[str, Any], force: bool = False) -> bool:
        ...
        try:
            ...
            cleaned_session_data = self._clean_session_data_for_archive(session_data)

            # 序列化完整会话数据（使用清理后的数据）
            session_json = json.dumps(cleaned_session_data, ensure_ascii=False)
    ...
    async def archive_search_session(
        ...
        try:
            ...
            #  v7.330: 提取深度分析相关字段（修复历史记录丢失）
            deep_analysis_result = search_result.get("deepAnalysisResult")
            four_missions_result = search_result.get("fourMissionsResult")
            quality_assessment = search_result.get("qualityAssessment")
            answer_thinking_content = search_result.get("answerThinkingContent", "")
            ...
            if existing:
                if force:
                    ...
                    #  v7.330: 更新深度分析相关字段（修复历史记录丢失）
                    if deep_analysis_result:
                        existing.deep_analysis_result = json.dumps(deep_analysis_result, ensure_ascii=False)
                    if four_missions_result:
                        existing.four_missions_result = json.dumps(four_missions_result, ensure_ascii=False)
                    if quality_assessment:
                        existing.quality_assessment = json.dumps(quality_assessment, ensure_ascii=False)
                    if answer_thinking_content:
                        existing.answer_thinking_content = answer_thinking_content
                    db.commit()
            ...
            archived = ArchivedSearchSession(
                ...
                #  v7.330: 深度分析相关字段（修复历史记录丢失）
                deep_analysis_result=json.dumps(deep_analysis_result, ensure_ascii=False)
                if deep_analysis_result
                else None,
                four_missions_result=json.dumps(four_missions_result, ensure_ascii=False)
                if four_missions_result
                else None,
                quality_assessment=json.dumps(quality_assessment, ensure_ascii=False) if quality_assessment else None,
                answer_thinking_content=answer_thinking_content,
            )
    ...
    async def get_search_session(self, session_id: str) -> Dict[str, Any] | None:
        ...
        try:
            ...
            search_result = {
                ...
                #  v7.330: 返回深度分析相关字段（修复历史记录丢失）
                "deepAnalysisResult": json.loads(session.deep_analysis_result)
                if getattr(session, "deep_analysis_result", None)
                else None,
                "fourMissionsResult": json.loads(session.four_missions_result)
                if getattr(session, "four_missions_result", None)
                else None,
                "qualityAssessment": json.loads(session.quality_assessment)
                if getattr(session, "quality_assessment", None)
                else None,
                "answerThinkingContent": getattr(session, "answer_thinking_content", "") or "",
            }
