    ...
    async def execute_step1_only(
        ...
        try:
            ...
            yield {
                ...
                "data": {
                    "message": "分析完成，请确认或编辑搜索方向",
                    "session_id": session_id,
                    "can_edit": True,
                    "framework_data": {
                        "original_query": framework.original_query if framework else query,
                        "core_question": framework.core_question if framework else "",
                        "answer_goal": framework.answer_goal if framework else "",
                        "boundary": framework.boundary if framework else "",
                        "targets": [t.to_dict() for t in framework.targets] if framework and framework.targets else [],
                        "framework_checklist": framework.framework_checklist.to_dict()
                        if framework and framework.framework_checklist
                        else None,
                    }
                    if framework
                    else None,
    ...
    async def execute_step2_only(
        ...
        try:
            while current_round < max_rounds:
                ...
                if not target:
                    logger.info(f"📋 [v7.280] 所有目标已完成，结束搜索")
                    break
                ...
                # 生成搜索查询
                search_query = await self._generate_search_query_smart(target, framework, all_sources, current_round)

                # 执行搜索
                sources = await self._search_web_with_fallback(search_query)
            ...
            # 调用答案生成
            async for event in self._generate_final_answer_stream(query, framework, all_sources):
                yield event
    ...
    def _rebuild_framework_from_user_edit(
        ...
        try:
            ...
            framework = SearchFramework(
                ...
                core_question=framework_data.get("core_question", query[:50]),
                answer_goal=framework_data.get("answer_goal", "为用户提供完整解答"),
                boundary=framework_data.get("boundary", ""),
            )

            # 优先从用户编辑的 checklist.main_directions 重建 targets
            ...
            logger.info(f"✅ [v7.280] 框架重建完成 | targets={len(framework.targets)}")
            return framework
        ...
        try:
            ...
            while current_round < max_rounds:
                ...
                if search_master_line and target is None:
                    ...
                    if current_task:
                        logger.info(
                            f" [v7.208] 执行任务 {current_task.id}: {current_task.task[:30]}... (P{current_task.priority})"
                        )
                        ...
                        framework.targets.append(target)
                        logger.info(
                            f" [v7.208] 创建临时搜索目标: {target.question or target.name or target.search_for or f'目标{target.id}'}"
                        )
                ...
                async for thinking_event in self._generate_unified_thinking_stream(
                    framework, target, current_round, all_sources, last_round_sources, last_search_query
                ):
                        ...
                        if current_round > 1 and unified_result and last_round_sources:
                            ...
                            yield {
                                ...
                                "data": {
                                    ...
                                    or target.name
                                    or target.search_for
                                    or f"目标{target.id}",  # v7.243: 使用备选字段
                ...
                if is_duplicate:
                    logger.info(
                        f" [Ucppt v7.199] 检测到重复查询: {search_query[:30]}... ≈ {similar_query[:30] if similar_query else ''}..."
                    )
                ...
                round_sources = await self._execute_search_with_quality_filter(search_query, target)
                ...
                # v7.227: 诊断日志 - 追踪搜索结果
                logger.info(
                    f" [round_sources v7.227] 准备发送 | round={current_round} | sources_count={len(round_sources)}"
                )
                if round_sources:
                ...
                if current_task and search_master_line:
                    ...
                    logger.info(
                        f" [v7.208] 任务 {current_task.id} 更新: status={current_task.status}, score={current_task.completion_score:.2f}"
                    )
                ...
                if force_continue:
                    ...
                    logger.info(
                        f" [v7.204] 强制模式: {target.question or target.name or target.search_for or f'目标{target.id}'} 状态={target.status}"
                    )  # v7.243
                ...
                logger.info(
                    f" [v7.230] 累积第{current_round}轮洞察: key_findings={len(round_insight.key_findings)}, insights={len(round_insight.inferred_insights)}, quality={round_insight.info_quality:.2f}"
                )
                ...
                if not force_continue and framework.get_extension_count() < MAX_EXTENSION_ROUNDS:
                    ...
                    if needs_extension and extension_points:
                        new_ext_targets = await self._add_extension_targets(framework, extension_points, current_round)
                        if new_ext_targets:
                            yield {
                                ...
                                "data": {
                                    ...
                                    "extensions": [{"name": t.name, "purpose": t.purpose} for t in new_ext_targets],
                ...
                if not force_continue:
                    ...
                    if (
                        not reflection_result.remaining_gaps
                        and reflection_result.alignment_with_goal >= 0.90
                        and reflection_result.estimated_rounds_remaining == 0
                    ):
                        yield {
                    ...
                    should_check_semantically = current_round % 5 == 0 or framework.overall_completeness >= 0.85
                    ...
                    if should_check_semantically:
                        ...
                        # 数值兜底检查
                        if (
                            framework.can_answer_completely()
                            and framework.overall_completeness >= self.completeness_threshold
                        ):
                            logger.info(
                                f"✅ [Ucppt v7.202] 数值兜底检查通过: 轮数{current_round}≥{MIN_SEARCH_ROUNDS}, 完成度{framework.overall_completeness:.2f}≥{self.completeness_threshold}"
                            )
                            yield {
                                "type": "ready_to_answer",
                                "data": {
            ...
            if extension_targets:
                ...
                async for ext_event in self._execute_serial_extensions(framework, all_sources, seen_urls):
                    yield ext_event
    ...
    def _generate_enhanced_keywords(
        ...
        if any(brand in query_lower for brand in ["hay", "丹麦", "北欧"]):
            if "民宿" in query_lower or "酒店" in query_lower:
                keywords.extend([f"HAY家具 中式空间 搭配案例", f"北欧极简风格 民宿设计 实际案例", f"现代丹麦家具 传统建筑 融合设计"])

        # 地点+风格融合关键词
        if any(loc in query_lower for loc in ["峨眉山", "四川", "山地"]):
            keywords.extend([f"四川山地建筑 现代家具 适应性设计", f"中国山区民宿 国际品牌家具 实用案例", f"松赞林卡 现代设计 本土材料 融合策略"])
...
请只输出JSON。"""
    ...
    def _parse_4_missions_result(self, data: Dict[str, Any], query: str) -> AnswerFramework:
        ...
        if not framework.key_aspects:
            logger.warning(f"⚠️ [v7.302] 未生成任何信息面，使用默认值")
            default_aspects = [
                ("核心概念", "解释基本定义和核心内容", 5),
                ("原因分析", "说明背后的原因和逻辑", 4),
                ("具体案例", "提供实际例子或数据支撑", 4),
