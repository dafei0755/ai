...
class SerperSearchTool:
    ...
    def search(
        ...
        **kwargs,
        ...
        try:
            ...
            # 处理响应
            logger.debug(f"⚙️ [Serper] Processing response...")
    ...
    def search_for_deliverable(
        ...
        try:
            ...
            # Step 1: 构建精准查询
            logger.debug(f"📝 [Serper Deliverable] Step 1: Building precise query...")
            ...
            if enable_qc and self.qc:
                logger.debug(f"🔬 [Serper Deliverable] Step 3: Running quality control...")
                qc_start = time.time()
            ...
            # Step 4: 添加编号（按质量分数排序）
            logger.debug(f"🔢 [Serper Deliverable] Step 4: Adding reference numbers...")
    ...
    def _process_search_response(self, response: Dict[str, Any], execution_time: float, query: str) -> Dict[str, Any]:
        ...
        if add_ids_to_search_results and processed["results"]:
            processed["results"] = add_ids_to_search_results(
                processed["results"], source_tool="serper"
            )
