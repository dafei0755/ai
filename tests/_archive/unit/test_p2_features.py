...
class TestDynamicRuleLoader:
    ...
    def test_update_threat_intelligence(self, rule_loader):
        ...
        # 更新威胁情报
        rule_loader.update_threat_intelligence(domains=test_domains, ips=test_ips, keywords=test_keywords)
...
class TestContentSafetyGuardWithDynamicRules:
    ...
    def test_enabled_disabled_categories(self, guard_with_dynamic_rules):
        ...
        if loader:
            ...
            # 检查是否有禁用的类别
            disabled_categories = [
                cat for cat, config in keywords.items() if isinstance(config, dict) and not config.get("enabled", True)
            ]
...
class TestConfigurationValidation:
    ...
    def test_detection_config_structure(self, config_path):
        ...
        # 检查关键配置项
        expected_keys = ["enable_keyword_check", "enable_privacy_check", "enable_evasion_check", "enable_external_api"]
