...
def test_precheck_node():
    ...
    # 不足信息的输入
    state_insufficient = {"user_input": "帮我设计一下房子", "session_id": "test-002", "processing_log": [], "node_path": []}
...
def test_conditional_routing():
    ...
    # 信息充足 → 应该进入 phase2
    state_go_phase2 = {"phase1_info_status": "sufficient", "recommended_next_step": "phase2_analysis"}

    decision1 = should_execute_phase2(state_go_phase2)
    print(f"✅ 信息充足时: {decision1} (预期: phase2)")

    # 信息不足 → 应该跳过 phase2
    state_skip_phase2 = {"phase1_info_status": "insufficient", "recommended_next_step": "questionnaire_first"}
...
def test_full_flow_mock():
    ...
    class MockLLM:
        ...
        def invoke(self, messages):
            ...
            if MockLLM.call_count == 1:
                ...
                response.content = json.dumps(
                    {
                        "analysis_layers": {"L1_facts": ["75平米公寓"], "L5_sharpness": {"score": 80}},
                        "structured_output": {
                            "project_task": "为32岁前金融律师设计75平米现代简约公寓",
                            "character_narrative": "追求品质生活的年轻专业人士",
                        },
                        "expert_handoff": {"critical_questions_for_experts": {"V3": ["如何表达专业身份？"]}},
                    },
                    ensure_ascii=False,
                )
    ...
    try:
        ...
        result = agent.execute(user_input="我是一位32岁的前金融律师，有一套75平米的公寓，预算60万，希望现代简约风格", session_id="test-full-flow")
