...
class TestGeoIPFallbackMechanism:
    ...
    def test_fallback_to_user_input(self):
        ...
        session = {
            ...
            "metadata": {"location": "未知", "geo_info": {"is_valid": False}},  # IP 定位失败
        }
    ...
    def test_fallback_chain(self):
        ...
        # 场景 1: IP 定位成功
        session1 = {"user_input": "我在北京", "metadata": {"location": "上海市", "geo_info": {"is_valid": True}}}  # IP 定位的结果
        location1 = session1["metadata"]["location"]
        assert location1 == "上海市"  # 使用 IP 定位结果

        # 场景 2: IP 定位失败，从用户输入提取
        session2 = {"user_input": "我在北京需要设计", "metadata": {"location": "未知", "geo_info": {"is_valid": False}}}
        cities = ["北京", "上海"]
        ...
        # 场景 3: 全部失败
        session3 = {"user_input": "我需要设计一个住宅", "metadata": {"location": "未知", "geo_info": {"is_valid": False}}}
...
class TestGeoIPPerformance:
    ...
    def test_batch_ip_lookup(self):
        ...
        test_ips = ["8.8.8.8", "1.1.1.1", "203.208.60.1", "127.0.0.1", "192.168.1.1"]
