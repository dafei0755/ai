...
class TestNodeInitialization:
    ...
    def test_questionnaire_summary_is_class(self, env_setup):
        """测试QuestionnaireSummaryNode是类 (🔧 v7.152: 替代已删除的RequirementsConfirmationNode)"""
        from intelligent_project_analyzer.interaction.nodes.questionnaire_summary import QuestionnaireSummaryNode

        assert isinstance(QuestionnaireSummaryNode, type)
...
class TestReviewNodesFunctionality:
    ...
    def test_questionnaire_summary_node_confirm(self, env_setup, mock_llm):
        """测试QuestionnaireSummaryNode确认功能 (🔧 v7.152: 替代已删除的RequirementsConfirmationNode)"""
        from datetime import datetime

        from intelligent_project_analyzer.interaction.nodes.questionnaire_summary import QuestionnaireSummaryNode

        # QuestionnaireSummaryNode 使用静态方法，不需要初始化
        # 测试类方法存在
        assert hasattr(QuestionnaireSummaryNode, "execute"), "QuestionnaireSummaryNode应该有execute方法"

        # 测试内部处理方法存在
        assert hasattr(
            QuestionnaireSummaryNode, "_handle_user_response"
        ), "QuestionnaireSummaryNode应该有_handle_user_response方法"
