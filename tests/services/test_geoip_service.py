...
class _FakeAddressNotFoundError(Exception):
    """用于测试的 GeoIP 地址缺失异常"""


sys.modules["geoip2.errors"].AddressNotFoundError = _FakeAddressNotFoundError
...
class TestGeoIPService:
    ...
    def test_get_client_ip_from_x_forwarded_for(self, geoip_service):
        ...
        mock_request = Mock()
        mock_request.headers.get = Mock(side_effect=lambda key: {"X-Forwarded-For": "1.2.3.4, 5.6.7.8"}.get(key))
    ...
    def test_get_client_ip_from_x_real_ip(self, geoip_service):
        ...
        mock_request = Mock()
        mock_request.headers.get = Mock(side_effect=lambda key: {"X-Real-IP": "9.10.11.12"}.get(key))
    ...
    def test_get_client_ip_priority(self, geoip_service):
        ...
        mock_request.headers.get = Mock(
            side_effect=lambda key: {"X-Forwarded-For": "1.1.1.1", "X-Real-IP": "2.2.2.2"}.get(key)
        )
...
class TestGeoIPServiceIntegration:
    ...
    def mock_fastapi_request(self):
        ...
        request.headers.get.side_effect = lambda key, default=None: headers_data.get(key, default)
        request.client = None
        return request
