...
class TestLoadBalancerConfig:
    ...
    def test_custom_config(self):
        """测试自定义配置"""
        config = LoadBalancerConfig(strategy="random", max_retries=5, rate_limit_per_key=200)
...
class TestOpenRouterLoadBalancer:
    ...
    def balancer(self, mock_keys):
        """创建负载均衡器实例"""
        return OpenRouterLoadBalancer(api_keys=mock_keys, model="openai/gpt-4o-2024-11-20")
    ...
    def test_load_keys_from_env(self):
        """测试从环境变量加载 Keys"""
        with patch.dict("os.environ", {"OPENROUTER_API_KEYS": "key1,key2,key3"}):
    ...
    def test_random_strategy(self, mock_keys):
        ...
        balancer = OpenRouterLoadBalancer(api_keys=mock_keys, config=config)
