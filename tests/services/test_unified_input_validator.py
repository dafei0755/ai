...
class TestUnifiedInputValidatorInitialValidation:
    ...
    def test_safe_design_input_high_confidence(self):
        """测试安全的设计类输入（高置信度）"""
        state = {"session_id": "test-session", "user_input": "我需要设计一个200平米的现代风格咖啡厅，位于商业街，目标客群是年轻白领"}
        ...
        result = UnifiedInputValidatorNode.execute_initial_validation(state, store=None, llm_model=mock_llm)
    ...
    def test_safe_design_input_low_confidence(self):
        ...
        state = {"session_id": "test-session", "user_input": "帮我做个东西"}  # 更模糊的输入，不包含设计关键词
    ...
    def test_non_design_input_high_confidence(self):
        """测试非设计类输入（高置信度）"""
        state = {"session_id": "test-session", "user_input": "帮我写一个Python爬虫程序"}
    ...
    def test_unsafe_content(self):
        """测试不安全内容"""
        state = {"session_id": "test-session", "user_input": "设计一个包含暴力血腥元素的空间"}
        ...
        with patch(
            "intelligent_project_analyzer.security.unified_input_validator_node.ContentSafetyGuard"
        ) as mock_guard:
...
class TestUnifiedInputValidatorSecondaryValidation:
        ...
        state = {
            ...
            "agent_results": {
                "requirements_analyst": {
                    "structured_data": {"project_task": "设计一个现代风格咖啡厅", "project_overview": "200平米商业街咖啡厅"}
                }
            },
        }

        result = UnifiedInputValidatorNode.execute_secondary_validation(state, store=None, llm_model=None)
    ...
    def test_secondary_validation_pass(self):
        ...
        state = {
            ...
            "agent_results": {
                "requirements_analyst": {
                    "structured_data": {"project_task": "设计一个办公空间", "project_overview": "500平米科技公司办公室"}
                }
            },
        }
        ...
        result = UnifiedInputValidatorNode.execute_secondary_validation(state, store=None, llm_model=mock_llm)
    ...
    def test_domain_drift_detection(self):
        ...
        state = {
            ...
            "needs_secondary_validation": True,
            "agent_results": {
                "requirements_analyst": {"structured_data": {"project_task": "开发一个网站", "project_overview": "电商平台前端开发"}}
            },
        }
...
class TestHelperMethods:
    ...
    def test_extract_project_summary_old_format(self):
        """测试提取项目摘要（旧格式）"""
        requirements_result = {
            "project_info": {"name": "咖啡厅设计", "type": "餐饮空间", "description": "现代风格咖啡厅"},
    ...
    def test_build_safety_rejection_message(self):
        """测试构造安全拒绝消息"""
        safety_result = {"violations": [{"category": "暴力血腥", "severity": "high"}]}
    ...
    def test_build_domain_guidance_message(self):
        """测试构造领域引导消息"""
        domain_result = {"detected_domain": "编程开发"}
