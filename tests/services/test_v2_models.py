...
class TestV2_0_FlexibleOutput:
    ...
    def test_subproject_brief_nested_model(self):
        ...
        subproject = SubprojectBrief(
            subproject_name="办公区", area_sqm=1500.0, key_requirements=["开放工位", "会议室", "休息区"], design_priority="高"
        )
    ...
    def test_targeted_mode_valid(self):
        ...
        data = {
            ...
            "targeted_analysis": {
                "coordination_strategy": {"key_principle": "统一中有变化", "approach": "建立设计语言体系，各子项目在此基础上适度变化"}
            },
        }
...
class TestV2_1_FlexibleOutput:
    ...
    def test_targeted_mode_valid(self):
        ...
        data = {
            ...
            "targeted_analysis": {
                "spatial_relationship": {"concept": "半开放式设计", "key_features": ["视觉通透", "功能独立", "互动友好"]}
            },
        }
...
class TestV2_OtherDirectors:
    ...
    def test_v2_3_office_space(self):
        ...
        data = {
            ...
            "targeted_analysis": {
                "flexibility_strategy": {"key_concept": "模块化可变空间", "design_elements": ["移动隔断", "多功能家具", "灵活布线"]}
            },
        }
    ...
    def test_v2_4_hospitality_space(self):
        ...
        data = {
            ...
            "targeted_analysis": {
                "atmosphere_creation": {"lighting": "低照度+重点照明", "materials": "温暖质感材料", "acoustics": "吸音处理，创造私密感"}
            },
        }
    ...
    def test_v2_5_cultural_space(self):
        ...
        data = {
            ...
            "targeted_analysis": {
                "public_space_strategy": {"circulation": "渐进式引导", "rest_areas": "多层次休息节点", "orientation": "清晰的导向系统"}
            },
        }
    ...
    def test_v2_6_architectural_landscape(self):
        ...
        data = {
            ...
            "targeted_analysis": {
                "integration_strategy": {"concept": "建筑生长于景观", "key_moves": ["延续地形", "引入绿化", "模糊边界"]}
            },
        }
...
class TestV2SeriesIntegration:
    ...
    def test_all_v2_models_use_decision_rationale(self):
        ...
        for model in models:
            model_fields = model.model_fields.keys()
            assert "decision_rationale" in model_fields, f"{model.__name__} should use 'decision_rationale'"
            assert "design_rationale" not in model_fields, f"{model.__name__} should NOT use 'design_rationale'"
    ...
    def test_comprehensive_mode_all_v2_models(self):
        ...
        v2_0_data = {
            ...
            "subproject_coordination": [
                {"subproject_name": "测试子项目", "key_requirements": ["需求1"], "design_priority": "高"}
            ],
