...
class TestV6_1_FlexibleOutput:
    ...
    def test_comprehensive_mode_valid(self):
        ...
        data = {
            ...
            "key_technical_nodes": [
                {"node_name": "大跨度梁", "challenge": "跨度过大，需要特殊设计", "proposed_solution": "采用预应力混凝土梁"}
            ],
    ...
    def test_comprehensive_mode_missing_fields(self):
        ...
        data = {
            ...
            "structural_system_options": [
                {"option_name": "框架结构", "advantages": ["灵活"], "disadvantages": ["刚度小"], "estimated_cost_level": "中"}
            ]
    ...
    def test_expert_handoff_response(self):
        ...
        data = {
            ...
            "design_rationale": "测试",
            "targeted_analysis": {"test": "data"},
            "expert_handoff_response": {"target_role": "V2-1", "handoff_content": "需要V2-1进一步细化空间设计"},
        }
    ...
    def test_challenge_flags(self):
        ...
        data = {
            ...
            "design_rationale": "测试",
            "targeted_analysis": {"test": "data"},
            "challenge_flags": [{"issue": "结构跨度过大", "severity": "must-fix", "suggestion": "建议增加中间支撑柱"}],
        }
...
class TestV6_2_FlexibleOutput:
    ...
    def test_targeted_mode_valid(self):
        ...
        data = {
            ...
            "targeted_analysis": {
                "hvac_optimization": {"current_system": "定风量系统", "proposed_system": "VAV变风量系统", "energy_savings": "30%"}
            },
        }
    ...
    def test_comprehensive_mode_valid(self):
        ...
        data = {
            ...
            "smart_building_scenarios": [
                {"scenario_name": "智能照明系统", "description": "自动调节，节能舒适", "triggered_systems": ["照明系统", "传感器系统", "中控系统"]}
            ],
...
class TestV6_3_FlexibleOutput:
    ...
    def test_node_detail_nested_model(self):
        """测试嵌套模型NodeDetail"""
        node = NodeDetail(
            node_name="石材干挂节点", challenge="大面积石材重量大，需确保连接强度", proposed_solution="采用不锈钢背栓+铝合金龙骨系统，单点承载≥300kg"
        )
    ...
    def test_targeted_mode_with_nested_models(self):
        ...
        data = {
            ...
            "targeted_analysis": {"material_recommendation": {"primary_material": "微水泥", "reason": "性价比高，施工周期短"}},
        }
    ...
    def test_comprehensive_mode_with_lists(self):
        ...
        data = {
            ...
            "critical_node_details": [{"node_name": "石材节点", "challenge": "重量大", "proposed_solution": "背栓系统"}],
...
class TestV6_4_FlexibleOutput:
    ...
    def test_cost_breakdown_nested_model(self):
        """测试嵌套模型CostBreakdown"""
        cost = CostBreakdown(category="装修工程", percentage=45, cost_drivers=["材料费占60%", "人工费占30%", "其他10%"])
    ...
    def test_ve_option_nested_model(self):
        """测试嵌套模型VEOption"""
        ve = VEOption(area="地面材料", original_scheme="进口大理石", proposed_option="国产优质石材", impact_analysis="节省成本30%，品质相当")
    ...
    def test_comprehensive_mode_with_nested_lists(self):
        ...
        data = {
            ...
            "cost_breakdown_analysis": [
                {"category": "装修工程", "percentage": 45, "cost_drivers": ["材料费60%", "人工费30%"]},
                {"category": "机电工程", "percentage": 30, "cost_drivers": ["设备费70%", "安装费20%"]},
            ],
            "value_engineering_options": [
                {"area": "地面材料", "original_scheme": "进口大理石", "proposed_option": "国产石材", "impact_analysis": "节省30%"}
            ],
...
class TestV6SeriesIntegration:
    ...
    def test_all_v6_models_importable(self):
        """测试所有V6模型可导入"""
        models = [V6_1_FlexibleOutput, V6_2_FlexibleOutput, V6_3_FlexibleOutput, V6_4_FlexibleOutput]
    ...
    def test_all_v6_models_have_required_fields(self):
        ...
        required_fields = ["output_mode", "user_question_focus", "confidence", "design_rationale"]

        for model in models:
            model_fields = model.model_fields.keys()
            for field in required_fields:
                assert field in model_fields, f"{model.__name__} missing required field: {field}"
    ...
    def test_all_v6_models_have_validator(self):
        ...
        for model in models:
            ...
            assert "validate_output_consistency" in validators or any(
                "validat" in v.lower() for v in validators
