...
class TestAnalysisAPI:
    ...
    def test_redis_session_manager_class(self):
        """测试RedisSessionManager类存在"""
        ...
        assert hasattr(RedisSessionManager, 'create')
        assert hasattr(RedisSessionManager, 'get')
        assert hasattr(RedisSessionManager, 'delete')
...
class TestToolsModule:
    ...
    def test_tavily_tool_exists(self):
        """测试Tavily工具模块存在"""
        # 简化：只验证模块可以导入
        try:
            from intelligent_project_analyzer.tools import tavily_search
            assert tavily_search is not None
        except ImportError:
            # 如果无法导入具体函数，至少验证模块存在
            import intelligent_project_analyzer.tools.tavily_search as tavily_module
            assert tavily_module is not None
