...
def test_deliverable_detection():
    ...
    for user_input, expected_level, expected_type in test_cases:
        ...
        if results:
            ...
            print(f"   实际: {first.capability_level.value} → {first.original_type if first.within_capability else first.transformed_type}")
...
def test_info_sufficiency():
    ...
    test_cases = [
        # (输入, 预期是否充足)
        (
            "我是一位32岁的前金融律师，有一套75平米的一居室公寓，预算60万，想要现代简约风格",
            True
        ),
        (
            "我想装修一下房子",
        ...
        (
            "帮我设计一个办公空间，200平米，预算100万，3个月工期，需要会议室和开放工位",
            True
        ),
        (
            "需要一个好看的设计",
